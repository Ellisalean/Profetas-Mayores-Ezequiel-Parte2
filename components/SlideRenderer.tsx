            ...
            <div className="flex flex-col lg:flex-row gap-10 items-stretch h-full">
               ...
               <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {slide.interaction?.revealItems?.map((item, i) => (
                    ...
                    }} className="flex items-center gap-5 p-6 bg-white/5 rounded-[2.5rem] border border-white/5 transition-all hover:bg-white/10 hover:border-red-600/40 text-left relative overflow-hidden group">
                       {viewedItems.has(i) && (
                         <div className="absolute top-2 right-2 text-red-500 opacity-50">
                           {renderIcon('CheckCircle', 12)}
                         </div>
                       )}
                       <div className="w-12 h-12 bg-white/10 rounded-2xl flex items-center justify-center text-white group-hover:bg-red-600 transition-colors">{renderIcon(item.icon, 24)}</div>
      ...
      case 'flashcards':
        return (
          <div className="w-full h-full flex flex-col p-6 lg:p-10">
             ...
             <div className="flex-1 flex flex-wrap gap-4 lg:gap-6 items-center justify-center py-2 max-w-7xl mx-auto w-full">
                {slide.interaction?.revealItems?.map((item, idx) => (
                  <div key={idx} onClick={() => toggleCard(idx)} className="perspective-1000 h-[320px] lg:h-[360px] w-full sm:w-[calc(50%-1rem)] lg:w-[calc(25%-1.5rem)] group cursor-pointer">
                    <div className={`relative w-full h-full transition-transform duration-700 transform-style-3d ${flippedCards[idx] ? 'rotate-y-180' : ''}`}>
                      <div className="absolute inset-0 backface-hidden bg-[#222222] rounded-[2.5rem] border border-white/10 overflow-hidden shadow-2xl flex flex-col">
                         ...
                         <div className="p-5 text-center space-y-2">
                            <h4 className="text-base lg:text-lg font-black uppercase tracking-tight text-white leading-tight">{item.title}</h4>
                            <span className="text-[7px] font-black text-red-500 uppercase tracking-widest block opacity-50">Toca para descubrir</span>
                         </div>
                      </div>
                      <div className="absolute inset-0 backface-hidden bg-red-600 rounded-[2.5rem] border border-white/20 shadow-2xl rotate-y-180 flex flex-col items-center justify-center p-6 text-center overflow-hidden">
                         <h4 className="text-[9px] font-black uppercase tracking-[0.3em] text-white/60 mb-3">Concepto Clave</h4>
                         <p className="text-sm lg:text-base font-bold text-white leading-relaxed mb-3">{item.text}</p>
                         <p className="text-xs text-white/80 font-light leading-relaxed">{item.longContent}</p>
                      </div>
                    </div>
                  </div>
                ))}
      ...
      case 'grid-cards':
        return (
          <div className="h-full w-full relative bg-white text-black overflow-hidden flex flex-col p-12 lg:p-20">
            ...
            <div className="relative z-10 flex-1 flex flex-wrap justify-center gap-8 items-start">
               {slide.interaction?.revealItems?.map((item, idx) => (
                  <div key={idx} className={`relative bg-white p-8 lg:p-6 rounded-sm shadow-[0_15px_40px_rgba(0,0,0,0.08)] border-t-4 border-white transition-all hover:shadow-2xl hover:border-red-600 group cursor-pointer w-full md:w-[calc(50%-2rem)] lg:w-[calc(25%-1.5rem)] max-w-md`} onClick={() => openGridPopup(item, idx)}>
                     {viewedItems.has(idx) && (
    ...
    <div className="h-full w-full relative flex flex-col overflow-y-auto custom-scrollbar bg-[#111111]">
      ...
      {expandedItem && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-6 lg:p-12">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setExpandedItem(null)} />
          <div className="relative w-full max-w-4xl bg-[#1a1a1a] shadow-2xl flex flex-col lg:flex-row overflow-hidden animate-in zoom-in-95 duration-500 rounded-[3rem] border border-white/10">
              <div className="lg:w-1/3 bg-black/20 flex flex-col items-center justify-center p-12 border-r border-white/5 relative">
                 <div className="text-red-600 mb-8 opacity-90">{renderIcon(expandedItem.icon, 120)}</div>
                 <h4 className="text-2xl font-black text-white uppercase tracking-tighter text-center leading-none">{expandedItem.title}</h4>
              </div>
              <div className="flex-1 p-12 lg:p-16 overflow-y-auto custom-scrollbar space-y-8 bg-[#1a1a1a] text-slate-200">
                 <div className="space-y-2">
                   <p className="text-xs font-black text-red-500 uppercase tracking-widest">Concepto Detallado</p>
                   <p className="text-xl lg:text-2xl font-bold leading-relaxed italic border-l-4 border-red-600 pl-8 bg-red-600/5 py-4 rounded-r-2xl">"{expandedItem.text}"</p>
                 </div>
                 <p className="text-lg lg:text-xl font-light leading-loose opacity-90 whitespace-pre-wrap">{expandedItem.longContent}</p>
                 <button onClick={() => setExpandedItem(null)} className="w-full py-4 bg-red-600 rounded-full font-black uppercase text-[10px] tracking-widest text-white hover:scale-105 transition-all">Entendido</button>
              </div>
              <button onClick={() => setExpandedItem(null)} className="absolute top-8 right-8 p-3 text-white/20 hover:text-red-500 transition-all hover:bg-white/5 rounded-full group">{renderIcon('X', 24)}</button>
          </div>
        </div>
      )}
